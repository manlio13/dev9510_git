...
begin
 ...
 try
  ...
  if DaysBetween(Now,inizio)>0 then
   ...
   Form1.Caption:='Controllo acquisti on line, ' + ' Versione : '+ GetAppVersionStr;
end;
...
begin
        if cambio=True then
        if MessageDlg('Hai dati da confermare ?',
...
begin
   if MessageDlg('Confermi di voler effettuare il backup del DataBase ?',
     ...
     begin
         ...
         ABSDatabase1.Close;
             if MessageDlg('Vuoi cambiare il percorso di salvataggio ?',
             mtconfirmation, [mbYes,mbNo], 0) = mrYes then
               begin
