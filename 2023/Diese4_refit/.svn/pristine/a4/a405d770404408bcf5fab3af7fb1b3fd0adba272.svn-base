...
procedure TForm1.Edit16KeyPress(Sender: TObject; var Key: Char);//consumo f(RPM)
   var
fcalc :double;
flo16:double; //RPM/1000
begin
      if (edit16.Text='')AND (combobox1.ItemIndex = -1)=true  then
        begin
          edit15.Text:= 'NA';
          showmessage('Please, Select engine type first');
          Edit16.Text:='';
          exit;
        end;
      if (Key = '.') OR (key =',') then    //controllo del tasto decimale
      ...
      begin                     //verifica se combobox1 seleziona il motore
        ...
        begin
          ...
          if combobox1.ItemIndex =4 then  //3JH2 Yanmar
            begin
              fcalc :=6.79564*intpower(flo16,2)-23.3952*flo16+21.9648;
              edit15.Text:= FormatFloat('#.##',fcalc);
            end;
           //scrive in edit15 il consumo orario per quel RPM

        end;
           Radiobutton3.Checked:=True;  //segna la modalità RPM
      end;
end;
...
begin
   if (Key = '.') OR (key =',') then    //controllo del tasto decimale
        if punto = mydecimal then
         key:= punto else key := virgola ;
   if (key=#13)  then
    if (StrToFloat(Edit3.Text)>0) then    //inserimento aliquota di variazione std
  begin
      edit3.Color:=clCream;
...
begin
  ...
  if (StrToFloat(Edit7.Text)>0) then
    begin
     ...
     if (Flag2 = true) AND (myini.Readstring('data','CumFill','')=inttostr(0))  then // attivato il change settings
...
begin                 //  entra le ore di motore
       if (RadioButton3.Checked=True) AND (Edit15.text='') then
       begin
         showmessage('Cannot fill any hour.'+#13#10+'Consumption rate missing');
         exit;
       end;
...
begin
    ...
    begin
      showmessage('    Attention'+#13#10+'Start from Engine Select'+#13#10+' then entry RPM');
