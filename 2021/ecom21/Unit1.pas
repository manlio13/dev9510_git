...
begin
 ...
 try
  ...
  if DaysBetween(Now,inizio)>0 then
   begin
      Label5.Caption:='€ '+FloatToStrF(Somma/(DaysBetween(Now,inizio)/30),ffFixed,4,2)+' per month';
      Label6.Caption:='N. items: '+IntToStr(ABSTable1.RecNo);
      somma:=0;
   end;
  
  end;

   Form1.Caption:='Control of online purchases, ' + ' Version : '+ GetAppVersionStr;
end;
...
procedure TForm1.Button4Click(Sender: TObject);   //sql   free text
begin
       DataSource1.DataSet:=ABSQuery1;
...
begin
      if messageDlg('You really want to erase all data ?',mtConfirmation,mbYesNo,0)=mrNo then
       exit;
      ABSTable1.Close;
...
begin
        if cambio=True then
        if MessageDlg('Data to be confirmed ?',
         mtconfirmation, [mbYes,mbNo], 0) = mrYes then Exit else cambio:=False;
...
begin
   if MessageDlg('Confirm that you want to backup the database ?',
             mtconfirmation, [mbYes,mbNo], 0) = mrYes then
     begin
         dirList:= TStringList.Create;      //inizializzare la string list
         ABSDatabase1.Close;
             if MessageDlg('Do you want to change backup path ?',
             mtconfirmation, [mbYes,mbNo], 0) = mrYes then
               begin
                  dir:=InputBox('Indicare il path','digitare l''intero percorso che termina per \','');  // creare la string
         ...
         ShowMessage('Backup done') else
           begin
              ShowMessage('Error ! Backup non done');
              Exit;
           end;
